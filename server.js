...
// Route for contacting openAI API to request a list of activities for give postcode
app.get('/activities', async (req, res) => {
    ...
    const completion = await openai.chat.completions.create({
        messages: [
            { role: "system", 
                content: `You are a helpful travelguide, event organiser and weather forecaster. 
                /n You will provide a brief description of the area based on the UK postcode which is: ${postcode} . 
                /n You will provide a brief forecast of the weather based on the weather information which is currently: ${weather} ${temperature} ${maxTemp}.
                /n suggest a range of activities to the user that can be done in the area ${name} and take into account the weather conditions.
                /n Provide a valid web link to any venue you suggest which must lead to a working website.
                /n do not mention covid19`
        },
...
// Route for contacting openAI API to ask more about the suggested activities and location
app.get('/chat', async (req, res) => {
    const userInput = JSON.stringify(req.query);
    const completion = await openai.chat.completions.create({
        messages: [
            { role: "system", content : `You are a helpful travelguide discussing possible activities to do in a local area given by the user`},
